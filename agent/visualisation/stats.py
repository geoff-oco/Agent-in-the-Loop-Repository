...
def stats_processing():
    ...
    try:
        ...
        return (total_phases, total_actions,
                (current_phase[0]),
                (current_phase[1]),
                (current_phase[2]))
...
def _phase_processing(game_data,phase):
    ...
    if blue_units > red_units:
        diff_units = "+" + str(blue_units - red_units)
    else:
        diff_units = blue_units - red_units

    if red_lost > blue_lost:
        diff_lost = "+" + str(red_lost - blue_lost)
    else:
        diff_lost = red_lost - blue_lost

    if blue_bases > red_bases:
        diff_bases = "+" + str(blue_bases - red_bases)
    else:
        diff_bases = blue_bases - red_bases
    
    

    return ((blue_units,blue_lost,blue_actions,blue_bases),
