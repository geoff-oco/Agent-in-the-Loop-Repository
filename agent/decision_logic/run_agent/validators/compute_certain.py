...
# Function to ensure that the next phase's base numbers, if no deletions, will equal their original adjusted by inserts
def compute_certain(
    ...
    # Phase index taken from our current phase state
    p = phase_index
    p1 = p + 1  # next phase index
    ...
    eff_orig_touched: Dict[str, bool] = (
        {}
    )  # This will track if our effective touched the base originally, marking it for change
    ...
    for tr in effective or []:
        ...
        to = str(tr.get("to"))
        addv = PhaseMath.vec(
            tr.get("L", 0), tr.get("H", 0), tr.get("R", 0)
        )  # The actual vectors for what is being moved
    ...
    all_bases = (
        set(start_orig_p1.keys())
        | set(end_p.keys())
        | set(inserts_to.keys())
        | set(inserts_from.keys())
        | set(eff_orig_touched.keys())
    )
    ...
    audit: Dict[str, Any] = (
        {}
    )  # This is our audit trail of how we got there, could be useful for debugging or display later
    ...
    for b in sorted(all_bases):
        ...
        all_inbound_ll = all(
            dmap.get(i, "leave") in ("leave", "lock") for i in inbound
        )  # Check if all inbound are leave or lock
        any_inbound_del = any(dmap.get(i, "leave") == "delete" for i in inbound)  # Check if any inbound are delete
        touched_by_orig = bool(eff_orig_touched.get(b, False))  # Check if original effective touched this base
        branch = None  # This will track which logic branch we took
        addv = PhaseMath.vec()
        subv = PhaseMath.vec()
        ...
        if (not touched_by_orig) and all_inbound_ll and (not any_inbound_del):
            ...
            # Deepcopy to avoid mutating our original data
            ...
            # Ensure we don't go negative with a clamp
            ...
            # If our candidate doesn't match the end snapshot we have an inconsistency likely from deletes affecting the base.
            # So we fallback to end snapshot
            if (not PhaseMath.eq(candidate["blue"], end_b.get("blue", PhaseMath.vec()))) or (
                not PhaseMath.eq(candidate.get("red", PhaseMath.vec()), end_b.get("red", PhaseMath.vec()))
            ):
                base_snap = end_b
    ...
    # Finally we resolve control situations where both sides are present
    ...
    for bname, sides in next_start.items():
        if (
            PhaseMath.sum_counts(sides.get("blue", PhaseMath.vec())) > 0
            and PhaseMath.sum_counts(sides.get("red", PhaseMath.vec())) > 0
        ):
