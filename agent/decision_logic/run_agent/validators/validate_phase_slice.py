...
# This validator checks and sanitises the model output for a phase slice, mainly decisions on original actions and any inserts
def validate_phase_slice(pdata: Dict[str, Any], model_out: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[str]]:
        #Our flags are used to determine valid moves and could be used in our logging
        flags: List[str] = []
        ...
        for d in (model_out.get("decisions") or []): #checking what we got for decisions from the model
            try:
                mid = int(d.get("id")) #model id must be int
        ...
        for i in orig_ids:
            decisions.append(md.get(i, {"id": i, "decision": "leave"})) #default to leave if missing
