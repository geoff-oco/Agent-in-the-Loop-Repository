...
class FinaliseSupport:
    ...
    def _fmt_side(s: Dict[str, Any]) -> str:
        return f'Blue {FinaliseSupport._fmt_counts(s.get("blue",{}))} | Red {FinaliseSupport._fmt_counts(s.get("red",{}))}'
    ...
    def format(runtime: Dict[str, Any], *, include_meta: bool = True) -> str:
        ...
        if include_meta and meta:
            ...
            if blue is not None and red is not None:
                ...
                lines.append(f"{blue} / {red} | Favour: {fav}")
                lines.append("")
    ...
    # Grab LER and favour and add to lines
        ...
        ler = meta.get("ler") or {}
        if fav:
            lines += [f"LER: {ler} in favour of: {fav}", ""]
