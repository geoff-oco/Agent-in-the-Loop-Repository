...
# Helper functions for our advise step
class AdviseSupport:

    # ----------- detail advise helpers ----------
    ...
    def build_runtime(game: Dict[str, Any]) -> Dict[str, Any]:
        ...
        runtime: Dict[str, Any] = {"meta": game.get("meta", {}), "phases": {}}
    ...
    def resolve_control(snapshot: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for bname, sides in list(out.items()):
            ...
            # Only if both sides are present do we need to resolve
            if PhaseMath.sum_counts(blue) > 0 and PhaseMath.sum_counts(red) > 0:
                out[bname] = AdviseSupport._resolve_control_one(sides, favour)
        return out

    # This method builds the full prompt for a given phase as its more complex than other prompts
    @staticmethod
    def build_phase_prompt(
        ...
        # Information for unit stats to help the model
        unit_hints = (
            "Unit hints: Light HP=4 (atk=2), Heavy HP=4 (atk=4), Ranged HP=1 (atk=1); "
            "use as intuition only; do not simulate battles."
        )

        # We build our preface to the phase prompt from the strategy, system prompt, advise instructions
        # and our previously generated summary for what we will do.
    ...
    # This method in simple compares two snapshots and infers the outcome for each base to assist the model
    @staticmethod
    def simple_infer_outcome(after_p: dict, before_p1: dict, favour: str = "") -> dict:
        ...
        for b in bases:
            ...
            # If either side is zero we have a clear winner already
            # If either side has more they win
            if bt > 0 and rt == 0:
                shift = "blue_control"
            elif rt > 0 and bt == 0:
                shift = "red_control"
