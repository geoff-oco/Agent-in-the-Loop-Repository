...
def node(state: ChatState) -> ChatState:
    ...
    #pdata is the original pgase data
    pdata = state.runtime["phases"][p]

    #grab our prompts and the summary to feed back in each phase
    ...
    #And call the LLM, we want json back
    ...
    # now we extract the json response
    try:
    ...
    #end_snapshot shows the end state of the phase after applying the decisions and inserts, eff shows the effective transfers, mflags shows any flags raised during math application
    #apply math is then used to apply our effective decisions all to get that end snapshot
    eff, end_snapshot, mflags = apply_math(
        pdata, decs, inserts, meta=(state.runtime.get("meta") or {})
    )

    # we update our phase with the important one being end data with the snapshot
    ...
    #certain is what we know for sure, possible is what could be true, audit is a full breakdown of how we got there, this will ensure valid moves from phase to phase are not overwritten
